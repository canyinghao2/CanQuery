...
public class ExpressageComAdapter extends NewBaseAdapter<ExpressageCompanyInfo > {
	...
	public ExpressageComAdapter(Context c, final List<ExpressageCompanyInfo > list,
		...
		ThreadHelper.getInstance().runAsync(new ThreadCallBack() {
			...
			public Object run(ThreadAsync arg0) {
				...
				for (int i = 0; i < list.size(); i++) {
					ExpressageCompanyInfo  info = list.get(i);
					String name = getAlpha(info.getNo());
	...
	public View getView(int position, View v, ViewGroup arg2) {
		...
		final ExpressageCompanyInfo  info = list.get(position);

		String currentStr = getAlpha(list.get(position).getNo());
		...
		cache.tv_city.setText(info.getCom());
		cache.tv_city.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				try {
					ExpressageCompanyInfo  ttinfo = DbUtils.create(
							App.getContext()).findFirst(
							Selector.from(ExpressageCompanyInfo .class)
									.where("type", "=", 1)
									.and("no", "=", info.getNo()));
